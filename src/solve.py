...
def main():
    ...
    while len(actions) > timestep:
        ...
        for a in actions[timestep]:
            # direction/state can be None before a train spawns or after it is
            # removed at its target, so look them up defensively
            agent = env.agents[a]
            direction = dir_map.get(agent.direction, '-')
            state = state_map.get(int(agent.state), agent.state)
            cmd = action_map.get(getattr(actions[timestep][a], 'value', actions[timestep][a]), actions[timestep][a])
            log.add(f'{a};{timestep};{agent.position};{direction};{state};{cmd}\n')
